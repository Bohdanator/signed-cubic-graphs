...
int singleout, lastout, recover, startvorgabe, bipartite; /* die Optionen */
int graph6_output=0;
int partial, timerestricted, TIMERESTRICTED, observer, short_code;    /* die Optionen */
int vertex_trans, independent, modulo, mod, rest;  /* die Optionen */
int transtest; /* eine variable, die beim Test auf vertextransitivitaet 
                   hilft */
int maxindep, splitlevel, minwrite;
...
void wegspeichern(liste,graphzahlen,graph)
...
if (!graph6_output && !noout)
 ...
 else if (noout) { number_of_graphs += graphzahlen[knotenzahl]; graphzahlen[knotenzahl]=0ULL; }
...
if (S_intervall)
      ...
      if (modulo) unsignedintegerschreib(splitzaehler,fil);
      fclose(fil);
      }
}
...
if (modulo && rest && (g00<minwrite)) return;

if (snarks) /* ist zwar ein teurer, aber auch sehr selektiver test */
    { 
     if (faerbbar(g)) return;
    }

if (zyconnecti)
    {
    if (cyc_con(g,&dummy)<zyminconn) return;
    }


if (graph6_output) { writeg6(g); return; }


/* oldcode1 und 2 werden abwechselnd benutzt, um rumkopieren zu vermeiden */
/* nach recover macht es auch nichts, wenn von 0 angefangen wird */

if (short_code)
    {
     if (whichcode[g00]) /* heisst : oldcode2 ist gueltig */
	 { shortcodiere(g,oldcode1[g00]);
	   for (i=0, p1=oldcode1[g00], p2=oldcode2[g00]; *p1 == *p2; p1++, p2++, i++);
	   *(nextposition[g00])=i+3; /* wieviel positionen stimmen ueberein ? */
	   (nextposition[g00])++;
	   while (i<codelength[g00]) {*(nextposition[g00]) = *p1; 
				      p1++; (nextposition[g00])++; i++; }
	   whichcode[g00]=0; }
     else  /* heisst : oldcode1 ist gueltig oder neustart */
	 { shortcodiere(g,oldcode2[g00]);
	   for (i=0, p1=oldcode2[g00], p2=oldcode1[g00]; *p1 == *p2; p1++, p2++, i++);
	   if (oldcode1[g00][0]==0) /* neustart */
	     { *(nextposition[g00])=0; (nextposition[g00])++;
	       *(nextposition[g00])=2; (nextposition[g00])++;
	       *(nextposition[g00])=3; (nextposition[g00])++;
	       *(nextposition[g00])=4; (nextposition[g00])++; }
	   else { 
	     *(nextposition[g00])=i+3; /* wieviel positionen stimmen ueberein ? */
	     (nextposition[g00])++; }
	   while (i<codelength[g00]) {*(nextposition[g00]) = *p1; 
...
else /* d.h. not short code */
  if (!noout)
    {
      codiere(g,nextposition[g00]);
      nextposition[g00] += codelength[g00];
    }

graphzahlen[g00]++;
 if (graphzahlen[g00]==(unsigned long long int)listenlaenge) 
    {     wegspeichern(globalliste,graphzahlen,g); 
	     }
}
...
if (modulo && adj[vertex]<3 && (graph[0][1]==splitlevel) && !recover)
    { splitzaehler++; if (splitzaehler== mod) splitzaehler=0;
      if (splitzaehler != rest) return; }


if (graph[0][1] > maximalkanten) maximalkanten=graph[0][1];

if ( (!singleout)  || recover  
...
if (flag && (!recover))
  {  
      if ((vertex<=knotenzahl) && (adj[vertex]>0) && (adj[vertex]<reg))
        { flag=0; wegspeichern(globalliste,graphzahlen,graph); }
...
if (vertex>knotenzahl)
                  { if (recover) { recover=0; }
                             ...
                             { if (independent)
                                ...
                                if (test) aufschreiben(graph);
                                for(i-- ; i>indepborder; i--) 
                                                  free(indeps[i]); }
                               else aufschreiben(graph); }
			 }
		  }
else
   {
   if (adj[vertex]==0)
        { if (recover) { recover=0; }
                    ...
                    { if (vertex<=border+2) aufschreiben(graph);
		      else if (endmini(graph,adj,1)) aufschreiben(graph); }
        }
   else
      if (adj[vertex]==reg) {
...
if (startvorgabe) { test=lesestartgraph(graph,adj,&startknoten); 
		    if (modulo && (splitlevel < graph[0][1]))
...
if (test) konstrukt(graph,adj,startknoten);
 ...
 wegspeichern(globalliste,graphzahlen,graph);
}
...
timerestricted=bipartite=recover=startvorgabe=singleout=lastout=partial=0;
observer=TIMERESTRICTED=vertex_trans=independent=maximalkanten=modulo=0;
...
if (argc < 3) 
 ...
 fprintf(stderr,"girth must be specified.\n"); exit(0); 
...
else { sscanf(argv[1],"%d",&knotenzahl);
       sscanf(argv[2],"%d",&taille); 
       for (i=3; i<argc; i++)
	   { switch(argv[i][0])
	     {
             case 'b': { bipartite=1; break; }
             case 'i': { independent=i;
                         singleout=1; /* fuer die kleineren werte wuerden
                                         durch die Belegung von five14s
                                         falsche Zahlen erzeugt */
                         maxindep = atoi(argv[i]+1);
                         break; }
	     case 'g': { graph6_output=1; break; }
             case 'm': { modulo=1; 
			 i++; rest=atoi(argv[i]); 
			 i++; mod=atoi(argv[i]);
			 if (rest>=mod) 
			 ...
			 init=0;
			 break; }
             case 'R': { remarks=1; break; }
             case 'r': { recover=1; break; }
             case 'c': { startvorgabe=i; /* gleich zum merken statt 1 */
                                   break; }
             case 'o': { lastout=1; break; }
             case 's': { if (argv[i][1]=='h') short_code=1;
			   else if (argv[i][1]==0) singleout=1; 
			     else {fprintf(stderr,"wrong option\n");
				   exit(0); }
			 break; }
	     case 'S': { snarks=1; break; }
             case 'v': { vertex_trans=1; break; }
             case 'p': { partial=i;
                                   break; }
             case 'P': {
   sprintf(strpuf1,"/etc/renice %s %d > yqvj ; rm yqvj ",argv[i]+1,getpid());
	     ...
	     case 'Z': { zyconnecti=i; zyminconn=atoi(argv[i]+1);
...
if (lastout || graph6_output) S_intervall=0; else S_intervall=Sicherungsintervall;
if (S_intervall) setup();

if (independent)
{
if (taille>3) { belegeminimalwerte();
                for (i=0; i<=knoten; i++) 
                     { if (i%14) { five14s[i]= 5*i;
                                   five14s[i]= (five14s[i]/14) +1; }
                             else five14s[i]= (5*i)/14; }
	      }
else if (maxindep>1)
{ for (i=0; i<=knoten; i++) if (i%3) five14s[i]= (i/3)+1;
                             else five14s[i]= i/3; } /* Der name passt dann
                                            natuerlich nicht gerade */

if (maxindep==1) { for (i=0; i<=knoten; i++) five14s[i]= 0; }
   }
...
{ 
switch(taille)
  { case 3: { splitlevel=20; break; }
    case 4: { splitlevel=22; break; }
    case 5: { splitlevel=24; break; }
    case 6: { splitlevel=29; break; }
    case 7: { splitlevel=34; break; }
    case 8: { splitlevel=40; /* just a guess */ 
	      fprintf(stderr,"contact gunnar@mathematik.uni-bielefeld.de whether the faster program for this task is ready!\n");
		      break; }
    case 9: { splitlevel=54; /* just a guess */
	      fprintf(stderr,"contact gunnar@mathematik.uni-bielefeld.de whether the faster program for this task is ready or better forget it!\n");
	      break; }
    case 10: { splitlevel=76; /* just a guess */
	      fprintf(stderr,"contact gunnar@mathematik.uni-bielefeld.de whether the faster program for this task is ready or better forget it!\n");
	      break; }
  default: { fprintf(stderr,"Girth too large. \n"); exit(1); }
}

if (bipartite && (taille<6)) splitlevel += 5;
if (independent) splitlevel +=3;
 if (forbiddencycles) splitlevel += 8;
 if (snarks) splitlevel+=3;

if ((3*knotenzahl)/2 <= splitlevel) splitlevel=(3*knotenzahl)/2-1;

minwrite= splitlevel*2/3+1;
...
if (knotenzahl>knoten) {fprintf(stderr,"Knotenzahl zu gross !\n\n"); exit(0);}
if ((taille < 3) || (taille > 10)) 
   {fprintf(stderr,"The girth must be between 3 and 10 !\n\n"); exit(0);}

for (i=4; i<=knoten; i+=2) { codelength[i]= (reg*i)/2-reg;
			     graphzahlen[i]=0ULL;
                             nextposition[i]=globalliste[i]=
                             (unsigned char*)malloc((codelength[i]+1)*listenlaenge+4);
 if (nextposition[i] ==NULL) { fprintf(stderr,"Can't get more space (15)-- exit\n"); exit(1); }
 }
...
if (singleout)
{   puffer= knotenzahl/2; puffer=puffer*3;
...
switch(taille)
  {
  case 3: startzahl=4; break;
  case 4: startzahl=6; break;
  case 5: startzahl=10; break;
  case 6: startzahl=14; break;
  case 7: startzahl=24; break;
  case 8: startzahl=30; break;
  case 9: startzahl=58; break; /* information by Brendan McKay */
...
codename[6]=(knotenzahl/10)+48; codename[7]=(knotenzahl%10)+48;
codename[12]=(taille/10)+48; codename[13]=(taille%10)+48;
if (bipartite) { i=strlen(codename); codename[i]='.'; codename[i+1]='b'; 
                 codename[i+2]=0; }
if (vertex_trans) { i=strlen(codename); codename[i]='.'; codename[i+1]='v'; 
                 codename[i+2]=0; }
...
if (modulo) { /* must be the last item added to the name */
                    sprintf(strpuf1,".m_%d_%d",rest,mod);
                    strcat(codename,strpuf1);
		  }
...
if (startvorgabe || partial) { sprintf(startgraphname,"startgraph"); 
                    strcat(startgraphname,codename+5); 
		    if (modulo)
			{ for (i=12; (startgraphname[i] != 'm') || 
				     (startgraphname[i-1] != '.'); i++);
			  startgraphname[i-1]=0; /*abschneiden*/ }
                    fil = fopen(startgraphname,"r");
                    if (fil==0) 
                      { fprintf(stderr,"No startfile %s there\n", startgraphname); exit(0); }
			   }



if (recover) {   fil = fopen(lastgraphname,"r");
                 if (fil == 0) { fprintf(stderr,"No lastgraph there ! \n");
                                exit(0); }
                 else fclose(fil); }
    else     
      if (S_intervall)
           { fil=fopen(lastgraphname,"w"); fclose(fil); }


if (partial) { sprintf(endgraphname,"endgraph"); 
...
{ 
codename[6]=(i/10)+48; codename[7]=(i%10)+48;
if (!graph6_output)
  {
    if (init) { fil = fopen(codename,"w"); fclose(fil); }
    else if ((fil = fopen(codename,"r"))!=NULL) 
      { fprintf(stderr,"There is a file %s. Is this OK ? (y/n)\n",codename);
	test=0; while ((test != 'y') && (test != 'n')) test=getchar();
	if (test=='n') exit(0); }
  }
}
...
if (recover)
{
   ...
   if (fread(lastgraph,1,sizeof(GRAPH),fil)<sizeof(GRAPH)) 
	 ...
	 recover=0; }
	      ...
	      if (modulo) unsignedintegerlies(&splitzaehler,fil);
	    }
   fclose(fil);
}
...
switch(taille)
  ...
  case 3: if (knotenzahl<16) border=knotenzahl-6; 
             else border=knotenzahl-7; break;
  case 4: border=knotenzahl-7; break;
  case 5: border=knotenzahl-8; break;
  case 6: border=knotenzahl-9; break;
  case 7:  if (knotenzahl<28) border=knotenzahl-12; 
             else border=knotenzahl-13; break;
  case 8: border=knotenzahl-17; break;
  case 9: border=knotenzahl-22; break; /* just a guess */
  case 10: border=knotenzahl-29; break; /* just a guess */
  }
